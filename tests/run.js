...
assert("catalog watch URLs are open-safe", ["cs", "nfl", "nba", "mlb", "nhl", "epl", "seriea", "ligue1", "laliga", "sumo", "gt"].every(function(id) {
  var info = SportsWatch.forSport(id)
  return SportsWatch.isOpenUrl(info.watch.url) && SportsWatch.isOpenUrl(info.updates.url)
}))
assert("fetch allowlist accepts ESPN", SportsWatch.isFetchUrl("https://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard"))
assert("fetch allowlist rejects random https", SportsWatch.isFetchUrl("https://evil.example/x") === false)
assert("open allowlist rejects userinfo", SportsWatch.isOpenUrl("https://evil@www.espn.com/") === false)
assert("open allowlist rejects lookalike host", SportsWatch.isOpenUrl("https://espn.com.evil.example/x") === false)
assert("open allowlist accepts ESPN game page", SportsWatch.isOpenUrl("https://www.espn.com/nfl/game/_/gameId/401/patriots-seahawks"))
assert("open allowlist allows YouTube handles", SportsWatch.isOpenUrl("https://www.youtube.com/@GTWorldChallengeEurope"))
